...
function initNavGesture() {
  ...
  _dbg.style.cssText = 'position:fixed;top:0;left:0;right:0;z-index:99999;padding:6px 10px;font-size:13px;font-weight:bold;color:#fff;pointer-events:none;transition:background 0.1s;';
  _dbg.textContent = 'v84 ready';
  document.body.appendChild(_dbg);
  function dbg(msg, color) { _dbg.textContent = msg; _dbg.style.background = color; }
  // ─────────────────────────────────────────────────────────
  ...
  function onMove(x) {
    ...
    for (const btn of btns) {
      ...
      btn.classList.toggle('active', x >= r.left && x <= r.right && btn.dataset.screen !== 'profile');
    }
  }
  ...
  nav.addEventListener('touchstart', e => {
    ...
    didDrag    = false;
    navActive  = true;
    dbg(`TS x=${Math.round(startX)}`, '#2563eb');
  }, { passive: true });

  // window touchmove with passive:false — the ONLY reliable way to intercept
  // horizontal drag on Android Chrome before the OS/browser claims it
  window.addEventListener('touchmove', e => {
    if (!navActive) return;
    e.preventDefault();
    const x = e.touches[0]?.clientX ?? startX;
    dbg(`TM x=${Math.round(x)} dx=${Math.round(x - startX)}`, '#059669');
    onMove(x);
  }, { passive: false });
  ...
  window.addEventListener('touchend', e => {
    ...
    dbg(`TE x=${Math.round(t.clientX)} drag=${isDragging}`, '#d97706');
