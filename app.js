...
function taskItem(t, compact = false) {
  ...
  <div class="task-item${t.done ? ' done' : ''}" data-id="${t.id}"
       onclick="openEditTask('${t.id}',event)">
...
function openEditTask(id, event) {
  ...
  openSheet(`
    <div style="display:flex;align-items:center;gap:12px;margin-bottom:20px">
      ...
      <button class="btn-primary" onclick="saveEditTask('${id}')" style="padding:8px 0;font-size:14px;margin:0;width:50%;flex-shrink:0">Сохранить</button>
    </div>
    <div class="form-group">
      <label class="form-label">Название</label>
    ...
    <div class="form-group">
      <label class="form-label">Заметка</label>
      <input class="form-input" type="text" id="fTaskNote" value="${esc(t.note)}" placeholder="Подробности...">
    </div>
...
function saveEditTask(id) {
  ...
  if (idx === -1) return;
  tasks[idx] = { ...tasks[idx], title, subject: $('fTaskSubject').value,
    deadline: $('fTaskDeadline').value, priority: $('fTaskPriority').value,
...
function initNavGesture() {
  ...
  let isDown      = false; // отслеживаем pointerdown
  let startX      = 0;
  let didDrag     = false;

  function getIndicator() { return document.getElementById('navIndicator'); }
  ...
  nav.addEventListener('pointermove', e => {
    if (!isDown) return; // только после pointerdown
    ...
    const navRect  = nav.getBoundingClientRect();
    const buttons  = [...nav.querySelectorAll('.nav-btn')].filter(b => b.offsetWidth > 0);
    if (!buttons.length) return;

    const firstR = buttons[0].getBoundingClientRect();
    const lastR  = buttons[buttons.length - 1].getBoundingClientRect();
    const btnW   = firstR.width;
    const minL   = firstR.left - navRect.left;
    const maxL   = lastR.right - navRect.left - btnW;
    const left   = Math.max(minL, Math.min(maxL, x - navRect.left - btnW / 2));

    const indicator = getIndicator();
    if (indicator) {
      indicator.style.left  = left + 'px';
      indicator.style.width = btnW  + 'px';
    }

    for (const btn of buttons) {
      const r = btn.getBoundingClientRect();
      if (x >= r.left && x <= r.right) {
        if (btn.dataset.screen && btn.dataset.screen !== 'profile' && btn.dataset.screen !== state.screen) {
          requestAnimationFrame(() => navigate(btn.dataset.screen));
        }
        break;
      }
    }
  ...
  nav.addEventListener('click', e => {
    if (didDrag) { didDrag = false; return; } // подавляем click после drag
